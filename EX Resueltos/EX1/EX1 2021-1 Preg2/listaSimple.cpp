...
Nodo* crearNodo( Elemento e, Nodo *s )
{
    ...
    p->elem.precio = e.precio;
    p->elem.numAcciones=e.numAcciones;
    p->elem.numOp=e.numOp;
    p->elem.tipoOp=e.tipoOp;
    strcpy(p->elem.fechHora, e.fechHora);
    strcpy(p->elem.tipoId, e.tipoId);
    p->elem.id = e.id;
    p->sig = s;
    return p;
}
