...
Nodo* crearNodo( Elemento e, Nodo *s )
{
    ...
    p->elem.hora = e.hora;
    p->elem.sucursal = e.sucursal;
    strcpy(p->elem.dia, e.dia);
    p->sig = s;
    return p;
}
...
void insertarComienzo( Lista &pL, Elemento e )
{
    ...
    if(!esListaVacia(pL)) pL.ultimo=p;
    pL.cabeza = p;
    pL.longitud++;
}
...
void mostrar( Lista L )
{
    ...
    else
    {
        ...
        while ( p!= NULL )
        {
            cout << p->elem.hora << "("<<p->elem.sucursal<<")->";
            p = p->sig;
        }
        cout << endl;
    }
...
Nodo* ubicarElemento( Lista L, Elemento e)
{
    ...
    else
    {
        ...
        while ( p != NULL )
        {
            if ( p->elem.sucursal == e.sucursal && p->elem.hora == e.hora && !strcmp(p->elem.dia, e.dia))
                return p;
            p = p->sig;    
        }
        return NULL;
