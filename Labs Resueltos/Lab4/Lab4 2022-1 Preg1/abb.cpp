...
void insertar( ABB &pA, Elemento e )
{
    ...
    else  // el árbol no está vacío
    {
        ...
        {
            pq = q;
            if ( e.camp1 < q->elem.camp1 || (e.camp1==q->elem.camp1 && e.camp2 < q->elem.camp2))
                q = q->hizq;
            else
                q = q->hder;
        }
        if ( e.camp1 < pq->elem.camp1 || (e.camp1==pq->elem.camp1 && e.camp2 < pq->elem.camp2))
            pq->hizq = p;
        else
            pq->hder = p;
    }
...
void mostrarEnOrden( NodoABB *pR )  // pR: puntero a la raiz del árbol
{
    ...
    if ( pR != NULL )
    {
        ...
        cout << "("<< pR->elem.camp1 << " " << pR->elem.camp2 << " " << pR->elem.camp3 << ") ";  // se visita la raiz
...
bool buscar( NodoABB *pR, Elemento e )
{
    ...
    while ( p != NULL )
    {
        if ( e.camp1 == p->elem.camp1 && e.camp2 == p->elem.camp2 )  // si lo encuentra retorna true
            return true;
        else
            if ( e.camp1 < p->elem.camp1 || (e.camp1==p->elem.camp1 && e.camp2 < p->elem.camp2) )
